...
class DataSaver:
    ...
    def save_json(self, data, file_name):
        """Saves JSON data to local disk or S3 using pre-configured paths."""
        if self.output_env == "local":
            ...
            self.s3_client.put_object(
                Bucket=self.s3_bucket, Key=s3_key, Body=json.dumps(data, indent=4), ContentType="application/json"
            )
...
def run_bronze_pipeline():
    ...
    metadata = api_handler.make_request(meta_url)
    if not metadata:
        logger.error("Failed to fetch metadata. Exiting.")
        raise Exception("Metadata fetch failed.")
    data_saver.save_json(data=metadata, file_name=global_settings["metadata_file_name"])
    ...
    for page in range(1, total_pages + 1):
        ...
        page_data = api_handler.make_request(bronze_settings["api_url"], params=api_params)
        ...
        data_saver.save_json(data=page_data, file_name=page_file_name)
